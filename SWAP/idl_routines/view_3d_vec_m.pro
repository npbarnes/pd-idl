...
case pln of
   ...
   'xy': begin
	 !x.title='x'
	 !y.title='y'
         surface,reform(arr(*,*,slcxy,vcomp)),charsize=1.5
    	 end;'xy'

   'xz': begin
	 !x.title='x'
	 !y.title='z'
         surface,reform(arr(*,slcxz,*,vcomp)),charsize=1.5
	 !x.title='x'
	 !y.title='y'
	 end;'xz'
   ...
   'yz': begin
	 !x.title='y'
	 !y.title='z'
         surface,reform(arr(slcyz,*,*,vcomp)),charsize=1.5
	 !x.title='x'
	 !y.title='y'
	 end;'yz'
endcase
...
clrb = findgen(sz(2))*numclr/max(findgen(sz(2)))
...
;     ytickname=[string(min(a)),string((min(a)+max(a))/2),string(max(a))], $
     ytitle='Density (10!u17!n ions/km!u3!n)',$
     yrange = [min(img)/1e17,max(img)/1e17]
...
case pln of
   ...
   'xy': begin
      ...
      if not((abs(max(vxy)) eq 0) and (abs(max(vyx)) eq 0)) then begin
         ...
         velovec,vxy,vyx,length=1.0, charsize=1.5
;                 title='max ='+string(max(sqrt(vxy^2 + vyx^2)))
   ...
   'xz': begin
      ...
      if not((abs(max(vxz)) eq 0) and (abs(max(vzx)) eq 0)) then begin
         ...
         velovec,vxz,vzx,length=1.0,charsize=1.5
;                 title='max ='+string(max(sqrt(vxz^2 + vzx^2))), $
   ...
   'yz': begin
      ...
      if not((abs(max(vyz)) eq 0) and (abs(max(vzy)) eq 0)) then begin
         ...
         velovec,vyz,vzy,length=1.0, charsize=1.5
...
case pln of
   ...
   'xy': begin
      ...
      if not((abs(max(vxy)) eq 0) and (abs(max(vyx)) eq 0)) then begin
         ...
         !p.title=''
;         !p.title=!p.title + ' (max = '+strtrim(string(max(sqrt(vxy^2 + vyx^2))),2)+')'
         img_velovec,img,vxy,vyx,x,y,length=1.0,charsize=1.4,/aspect,$
   ...
   'xz': begin
      ...
      if not((abs(max(vxz)) eq 0) and (abs(max(vzx)) eq 0)) then begin
         ...
         !p.title=''
;         !p.title=!p.title + ' (max = '+strtrim(string(max(sqrt(vxz^2 + vzx^2))),2)+')'
         img_velovec,img,vxz,vzx,x,z,length=1.0,charsize=1.4,/aspect, $
   ...
   'yz': begin
	 ...
	 !p.title=''
;         !p.title=!p.title + ' (max = '+strtrim(string(max(sqrt(vyz^2 + vzy^2))),2)+')'
         img_velovec,img,vyz,vzy,y,z,length=1.0,charsize=1.4,/aspect, $
              title=ptit+' (max = '+strtrim(string(max(sqrt(vyz^2 + vzy^2))),2)+')'
         !x.title='x'
         !y.title='y'
      endif
      end;yz
...
xmenu,['np','nf'],mctr,/row,/exclusive,$
      space=1,uvalue=['NP','NF']
