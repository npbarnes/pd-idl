...
;readu,1,ny
;readu,1,nz
readu,1,nx,ny,nz,idn,ll
;print,nt,nout,nx,ny,nz
print,nx,ny,nz,idn,ll

temparr = dblarr(nx,ny,nz,3,/nozero)
cnt=1
while not(eof(1)) do begin
   readu,1,frame
...
if (flg eq 0) then begin
   ...
   varr = temparr(minx:maxx,miny:maxy,minz:maxz,*)
   arr = varr(*,*,*,vcomp)
endelse   
...
case pln of
   ...
   'xy': begin
	 !x.title='x'
	 !y.title='y'
         surface,reform(arr(*,*,slcxy)),charsize=1.0
    	 end;'xy'

   'xz': begin
	 !x.title='x'
	 !y.title='z'
         surface,reform(arr(*,slcxz,*)),charsize=1.0
	 !x.title='x'
	 !y.title='y'
	 end;'xz'
   ...
   'yz': begin
	 !x.title='y'
	 !y.title='z'
         surface,reform(arr(slcyz,*,*)),charsize=1.0
	 !x.title='x'
	 !y.title='y'
	 end;'yz'
endcase
...
;     ytickname=[string(min(a)),string((min(a)+max(a))/2),string(max(a))], $
     ...
     yrange = [min(img)/1e15/1e6,max(img)/1e15/1e6]
...
case pln of
   ...
   'xy': begin
      ...
      vxy = reform(varr(*,*,slcxy,0))
      vyx = reform(varr(*,*,slcxy,1))
...
;                 title='max ='+string(max(sqrt(vxy^2 + vyx^2)))
   ...
   'xz': begin
      ...
      vxz = reform(varr(*,slcxz,*,0))
      vzx = reform(varr(*,slcxz,*,2))
...
;                 title='max ='+string(max(sqrt(vxz^2 + vzx^2))), $
   ...
   'yz': begin
      ...
      vyz = reform(varr(slcyz,*,*,1))
      vzy = reform(varr(slcyz,*,*,2))
...
case pln of
   'xy': anmt_arr = dblarr(nx,ny,nfrm)
   'xz': anmt_arr = dblarr(nx,nz,nfrm)
   'yz': anmt_arr = dblarr(ny,nz,nfrm)
endcase
...
case pln of
   ...
   'xz': begin
      get_movie_frames
      arr1=bytscl(anmt_arr)
      arr2=congrid(arr1,512,483,nfrm)
      xinteranimate,set=[512,483,nfrm]
      for i=0,nfrm-1 do begin
         xinteranimate,image=arr2(*,*,i),frame=i
      endfor
      xinteranimate
...
base = widget_base(title='Hydrodynamics 3-D plotter',/row)
...
xmenu,['surface','contour','vector','img_vector','np profile'],$
      rct,/column, $
      /exclusive,title='graphics options',space=50,$
      uvalue=['SURFACE','CONTOUR','VECTOR','IMG_VECTOR','PROFILES']

w1 = widget_label(rcanim,value = 'Animations')
w1 = widget_button(rcanim,value = 'velocity',uvalue = 'ANIMATE')
...
xmenu,['u','T','n','p'],mctl,/row,/exclusive,$
      space=1,uvalue=['U','T','N','P']
