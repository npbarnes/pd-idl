...
;--------------------------------------------------------------------------
PRO vect_slice,v,tit,sxy,sxz,syz
...
;   window,1,title='xy',xsize = 400, ysize = 400
;   !p.position(0)= 0.1
;   !p.position(1)= 0.1
;   !p.position(2)= 0.9
;   !p.position(3)= 0.9
   velovec,vxy,vyx,x,y,xtitle='x',ytitle = 'y',length=1.0, $
                       title=tit+': max ='+string(max(sqrt(vxy^2 + vyx^2)))
endif

if not((abs(max(vxz)) eq 0) and (abs(max(vzx)) eq 0)) then begin
;   window,2,title='xz',xsize = 400, ysize = 400
;   !p.position(0)= 0.1
;   !p.position(1)= 0.1
;   !p.position(2)= float(nx)/float(nz)
;   !p.position(3)= 0.9
   velovec,vxz,vzx,x,z,xtitle='x',ytitle = 'z',length=1.0, $
                       title=tit+': max ='+string(max(sqrt(vxz^2 + vzx^2)))
endif

if not((abs(max(vyz)) eq 0) and (abs(max(vzy)) eq 0)) then begin
;   window,3,title='yz',xsize = 400, ysize = 400
   velovec,vyz,vzy,y,z,xtitle='y',ytitle='z',length=1.0, $
                       title=tit+': max ='+string(max(sqrt(vyz^2 + vzy^2)))
endif

;window,4,title='z profile',xsize=400,ysize=400
;plot,vxz(4,*)
