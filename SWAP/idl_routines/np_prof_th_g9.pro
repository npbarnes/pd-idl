;-------------------------------------------------------------------
pro np_prof_th_g9,nfiles,nfrm,maxnp,POSTSCRIPT=postscript
;-------------------------------------------------------------------
...
;      !x.tickv = [min(x),max(x)]
	 ...
	 !x.ticks = 3
	 !x.tickv = [0,10,20,30]
         !x.tickname = ['0','10','20','30']
      endif 
      if (frm_cnt eq nfrm/2) then !y.title = '10!e23!n ions per 0.4 km'
      plot,x,nprof, charsize = 1.2,/noerase
      tm = strmid(strtrim(string(0.2+(frm_cnt)*50*0.004),2),0,3)
