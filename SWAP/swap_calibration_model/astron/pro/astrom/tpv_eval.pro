...
if Npv1 GT 7 then begin
  ...
  if Npv1 GT 12 then begin
      ...
      if Npv1 GT 13 && (pv1[13] NE 0.0) then xp += pv1[13]*y2*y*x
      if Npv1 GT 14 && (pv1[14] NE 0.0) then xp += pv1[14]*y2*x2
      if Npv1 GT 15 && (pv1[15] NE 0.0) then xp += pv1[15]*y*x2*x
      if Npv1 GT 16 && (pv1[16] NE 0.0) then xp += pv2[16]*x2*x2
      endif
 endif      

yp = pv2[0] + pv2[1]*y + pv2[2]*x
if Npv1 GT 3 && (pv1[3] NE 0.0) then yp += pv2[3]*sqrt(x2 + y2)
...
if NPv2 GT 7 then begin
  ...
  if NPv2 GT 12 then begin
      if (pv2[12] NE 0.0) then yp += pv2[12]*x2*x2
      if NPv2 GT 13 && (pv2[13] NE 0.0) then yp += pv2[13]*x2*x*y
      if NPv2 GT 14 && (pv2[14] NE 0.0) then yp += pv2[14]*x2*y2
      if NPv2 GT 15 && (pv2[15] NE 0.0) then yp += pv2[15]*x*y2*y
      if NPv2 GT 16 && (pv2[16] NE 0.0) then yp += pv2[16]*y2*y2
      endif
